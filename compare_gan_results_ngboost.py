"""
NGBoost GAN Comparison Script
Trains NGBoost on three datasets:
...
REPORT_PATH = r'c:\Users\shoai\Desktop\alternate 4\ngboost_gan_comparison_report.txt'

# NGBoost params — identical to train_ngboost.py
NGB_PARAMS = dict(
    Dist=Bernoulli,
    n_estimators=100,
    learning_rate=0.01,
...
def train_and_evaluate(data_path, label):
    """Train NGBoost on a dataset and return metrics."""
    print(f"\n{'='*60}")
    print(f"  Training NGBoost on: {label}")
    print(f"{'='*60}")
...
def main():
    ...
    # Build comparison
    print(f"\n{'='*80}")
    print("  NGBoost COMPARISON: No GAN vs Standard GAN vs CGAN")
    print(f"{'='*80}\n")
